...
class NaverWebtoonCrawler:
    ...
    def make_index_html(self):
        if not os.path.isdir('db'):
            return False
        downloaded_webtoons = os.listdir('db')
        downloaded_webtoons = set(downloaded_webtoons)
        webtoon_list = self.get_webtoon_list()
        webtoons_info = [[webtoon.title, webtoon.title_id, webtoon.img_url] for webtoon in webtoon_list if
                            webtoon.title_id + '.txt' in downloaded_webtoons]
        content = ''
        for webtoon in webtoons_info:
            content += '''\t\t<div class="webtoon">
    \t\t<a href="{webtoon_link}" class="img"><img src="{img_url}"></a>
    \t\t<a href="{webtoon_link}" class="title">{title}</a>
\t\t</div>\n'''.format(webtoon_link='webtoon/' + webtoon[1] + '.html', img_url='db/' + webtoon[1] + '.jpg', title = webtoon[0])
        with open('index.html', 'wt') as f:
            index = open('html/index.html', 'rt').read()
            index = index.replace('*content*', f'{content}')
            f.write(index)
    ...
    def save(self, path=None):
        ...
        # 웹툰 대표 썸네일 하나도 같이 저장
        response = requests.get(self.webtoon.img_url)
        filepath = f'db/{self.webtoon.title_id}.jpg'
        ...
        with open(filename, 'wt') as f:
            ...
            for e in self.episode_list:
                ...
                f.write(list_html_tr.format(
                    page_link = f'./{self.webtoon.title_id}_images/{e.no}.html',
...
if __name__ == '__main__':
    crawler = NaverWebtoonCrawler('군인')
